...
const url_head = encodeURIComponent(
  "https://hk4e-api-os.hoyoverse.com/event/gacha_info/api/getGachaLog?authkey_ver=1&sign_type=2&auth_appid=webview_gacha&init_type=301&lang=en&authkey="
);
let apikey;
...
async function wish(
  ...
  let url_end = encodeURIComponent(
    "&gacha_type=" + gacha_type + "&page=" + page_num + "&size=5&end_id=" + end_id
  );
  ...
  if (response && response.data && response.data.list) {
    ...
    if (newwishinfor.length !== 0) {
      if (newwishinfor.length < 5) {
        for (let index = 0; index < newwishinfor.length; index++) {
          get_wish_history = get_wish_history.concat(newwishinfor[index]);
        }
        if (gacha_type === 301) {
          page_num = 1;
          end_id = 0;
        ...
        wish_loop = newwishinfor;
        get_wish_history = get_wish_history.concat(newwishinfor);
        if (page_num === 1) {
          page_num++;
          ...
          return new Promise((resolve) => {
            setTimeout(async () => {
              await wish(pointer, page_num, gacha_type, end_id, response);
              resolve();
            }, 50);
          });
        }
    ...
    if (response.message == "visit too frequently") {
      error = "Error: Visit too frequently, try again later";
      fetch_status = "Import terminated";
      banner_name.set("");
      loading = false;
      return error;
    } else if (response.message == "authkey error") {
...
async function map() {
  ...
  if (Uniquedata.length === 0) {
    ...
    const inserts = Uniquedata.map((d) =>
      prisma.wish_history.create({
        data: {
          name: d.name,
          gacha_id: d.gacha_id,
...
//                         gacha_type = 302;
//                         console.log('from 301 to 302');
//                         return await wish(pointer = "wish_history", page_num, gacha_type, end_id, response)
//                     }
//                     else if (gacha_type===302) {
//                         page_num = 1;
//                         end_id = 0;
...
//     }
//     else{
//         const inserts = Uniquedata.map(d => await prisma.wish_history.create({
//             data:{
//               name: d.name,
//               gacha_id: d.gacha_id,
//               gacha_type: d.gacha_type,
